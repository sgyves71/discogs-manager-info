...
export function getStageMusicBrainzCatalogContext(criteria: MusicBrainzSearchCriteria): MusicBrainzCatalogContext {
  ...
  return {
    artist: results.artists[0] ? {
      ...
      annotation: 'Stage Mock Artist is a synthetic group used to verify the MusicBrainz artist-summary fallback order.', genres: ['heavy metal'],
    } : null,
    releaseGroup: results.releaseGroups[0] ? {
      ...
      annotation: 'This synthetic album annotation represents MusicBrainz release-group notes.', genres: ['heavy metal'],
    } : null,
  };
}
