...
- Catalog entries can now persist an optional, user-selected personal album-folder mapping. The path must be an existing, indexed folder under the configured music-library root. Local track playback prefers that folder and matches its track title directly; mappings can be set from suggested folder results, pasted manually, or cleared from catalog details.
