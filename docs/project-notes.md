...
- Catalog entries can persist an optional, user-selected personal album-folder mapping. The recovery picker appears only after a personal-track playback lookup fails; it uses suggested results or an in-app scan-index picker, and can clear an existing mapping during that recovery flow. Local playback prefers a saved folder and matches its track title directly.
