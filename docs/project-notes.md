...
- Catalog entries can persist an optional, user-selected personal album-folder mapping. When a personal-track playback lookup fails, the app immediately displays a Yes/No dialog offering a manual match. Choosing Yes opens the recovery picker with suggested results and an in-app scan-index picker; it can also clear an existing mapping during that recovery flow. Local playback prefers a saved folder and matches its track title directly.
- Manual personal-folder mapping uses an in-app picker populated solely from the existing music-library scan index: choose a base artist folder, then one of its indexed album folders. Manual path entry is not needed; browsing is constrained to the configured music-library root.
