...
- Catalog entries can now persist an optional, user-selected personal album-folder mapping. The path must be an existing, indexed folder under the configured music-library root. Local track playback prefers that folder and matches its track title directly; mappings can be set from suggested folder results or cleared from catalog details.
- Manual personal-folder mapping now uses an in-app picker populated solely from the existing music-library scan index: choose a base artist folder, then one of its indexed album folders. Manual path entry is no longer needed; browsing is constrained to the configured music-library root.
